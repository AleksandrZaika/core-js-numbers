...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const angleCosine = dotProduct / (magnitude1 * magnitude2);
...
function getFibonacciNumber(index) {
  ...
  for (let i = 2; i <= index; i += 1) {
    fibonSeq[i] = fibonSeq[i - 1] + fibonSeq[i - 2];
  }

  return fibonSeq[index];
}
...
function getCountOfOddNumbers(number) {
  ...
  const countOfNumbers = Math.floor((absoluteNumber + 1) / 2);
